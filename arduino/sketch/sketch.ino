...
void loop() {
  if (Serial.available()) {
    ...
    if (strncmp(readbuffer, "AT", len) == 0) {
      Serial.println("OK");
    }
    else if (strncmp(readbuffer, "AT+RESET", len) == 0) {
      WiFi.mode(WIFI_STA);
      WiFi.disconnect();
      Serial.println("OK");
    }
    else if (strncmp(readbuffer, "AT+GET_MAC", len) == 0) {
      uint8_t macAddr[6];
      WiFi.macAddress(macAddr);
      Serial.printf("%02x:%02x:%02x:%02x:%02x:%02x\n", macAddr[0], macAddr[1], macAddr[2], macAddr[3], macAddr[4], macAddr[5]);
    }
    else if (strncmp(readbuffer, "AT+LIST_AP", len) == 0) {
    ...
    else if (strncmp(readbuffer, "AT+DISCONNECT", len) == 0) {
      WiFi.disconnect();
      Serial.println("OK");
    }
    else if (strncmp(readbuffer, "AT+IS_CONNECTED_TO_AP", len) == 0) {
      bool b = WiFi.isConnected();
      if (b) Serial.println("YES");
      else Serial.println("NO");
    }
    else if (strncmp(readbuffer, "AT+CONNECTED_AP", len) == 0) {
      Serial.printf("%s\n", WiFi.SSID().c_str());
    }
    else if (strncmp(readbuffer, "AT+GET_IP", len) == 0) {
      if (WiFi.status() == WL_CONNECTED)
        Serial.println(WiFi.localIP());
      else Serial.println("NOT CONNECTED");
    }
    else if (strncmp(readbuffer, "AT+INIT_TLS", len) == 0) {
    ...
    else if (strncmp(readbuffer, "AT+TLS_CLOSE", len) == 0) {
      client.stop();
      Serial.println("OK");
    }
    else {
